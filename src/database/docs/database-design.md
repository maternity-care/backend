...
- `users`: tài khoản khách hàng, chỉ dùng cho role `member` và `partner`; đăng nhập qua user auth.
- `staffs`: tài khoản nội bộ độc lập, gồm super admin, admin cơ sở, bác sĩ, điều dưỡng và staff; có email/password riêng và đăng nhập qua management auth.
...
- `user_roles`: bảng nối role khách hàng với user. Chỉ dùng cho `member`, `partner`.
- `staff_roles`: role toàn hệ thống của staff, hiện dùng cho `super_admin`.
- `role_permissions`: bảng nối nhiều-nhiều giữa role và permission. Dùng để cấu hình role nào được phép gọi chức năng nào.
- `user_permissions`: bảng override quyền theo từng user. Dùng để cấp thêm quyền riêng hoặc tắt một số quyền user đang được hưởng từ role.
...
- `doctors`: hồ sơ chuyên môn bổ sung, liên kết trực tiếp với `staffs`.
...
- `facility_staff`: quan hệ mọi nhân viên, bao gồm bác sĩ, làm việc tại cơ sở nào và giữ `role_id` tại cơ sở đó. Permission hiệu lực lấy từ role của cơ sở đang chọn.
...
- Dashboard/report không cần bảng riêng ở MVP. Các thống kê doanh thu, lịch khám, dịch vụ, bác sĩ, sản phẩm bán chạy nên query từ `orders`, `payments`, `appointments`, `medical_records`.
...
Mục đích: lưu override permission trực tiếp trên user, cho phép bật thêm quyền riêng hoặc tắt quyền user được hưởng từ role.
...
| --- | --- | --- |
| id | bigint | PK |
| staff_id | bigint | FK staffs, unique |
...
Mục đích: lưu hồ sơ nhân sự chung của admin cơ sở, bác sĩ, điều dưỡng, lễ tân, điều phối và kế toán. Bác sĩ có thêm hồ sơ chuyên môn trong `doctors`.

| Column | Type | Note |
| --- | --- | --- |
| id | bigint | PK |
| name | varchar(150) |  |
| email | varchar(190) | Unique, email công ty |
...
Mục đích: gán mọi nhân viên, bao gồm bác sĩ, vào cơ sở để phân quyền và lọc nhân sự theo từng chi nhánh.

Gán nhân viên vào một hoặc nhiều cơ sở.

| Column | Type | Note |
| --- | --- | --- |
| facility_id | bigint | PK, FK facilities |
| staff_id | bigint | PK, FK staffs |
| role_id | bigint | PK, FK roles; một nhân viên có thể giữ nhiều role tại cùng cơ sở |
| status | varchar(30) | `active`, `inactive` |
